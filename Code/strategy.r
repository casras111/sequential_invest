...
beststrat <- function(DDate,ksearch,l,window_th=10,recent_weight=FALSE) {
  ...
  xvec_k <- matrix(rep(0,(nstocks-1)*length(ksearch)),nrow=length(ksearch)) 
  ...
  maxret_mat <- matrix(rep(maxret_vec,(nstocks-1)),nrow=length(ksearch),byrow=F)
  ...
  names(xvec) <- colnames(StockPrices)[retcolnames[-nstocks]]
  #0.01 used to prevent calculation precision errors when comparing to 0
